-- Update Customer Database Schema for Email Verification
-- Run this SQL in your Supabase SQL editor to add email verification fields

-- Add verification columns to customers table if they don't exist
DO $$ 
BEGIN
    -- Add status column if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'customers' AND column_name = 'status') THEN
        ALTER TABLE customers ADD COLUMN status TEXT NOT NULL DEFAULT 'unverified' CHECK (status IN ('unverified', 'verified'));
    END IF;
    
    -- Add verification_token column if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'customers' AND column_name = 'verification_token') THEN
        ALTER TABLE customers ADD COLUMN verification_token TEXT;
    END IF;
    
    -- Add verification_token_expires_at column if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'customers' AND column_name = 'verification_token_expires_at') THEN
        ALTER TABLE customers ADD COLUMN verification_token_expires_at TIMESTAMP WITH TIME ZONE;
    END IF;
...
UPDATE customers SET 
    status = 'verified',  -- Mark existing customers as verified
    login_count = 0 
WHERE status IS NULL OR login_count IS NULL;
